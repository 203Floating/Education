<template>
  <div :class="$style.container">
    ...
    <el-scrollbar class="custom-scrollbar"> </el-scrollbar>

    <el-dialog v-model="dialogVisible" title="添加连选课程" :class="$style.dialog">
...
const toSaveCourse = async () => {
  try {
    ...
    const deleteRes = await useDeleteData('http://localhost:3000/course/link/delete', {
      cs_id: id
    })
    if (deleteRes.data.status) {
      const savePromises = checkSub.value.map(async (item) => {
        await usePostData('http://localhost:3000/course/link', {
          cs_id: id,
          c_ids: item
        })
      })
